...
class PHPWhois {
    ...
    public static function whoisservers($tld = null) {

      if($tld == null) {
          throw new \Exception("TLD should be passed");
      }
...
public static function LookupDomain($domain){
    ...
    $code = '0';
    $result = 'ERROR';
    $message = 'Initalizing...'.$domain;
	$domain_parts = explode(".", $domain);
	$tld = strtolower(array_pop($domain_parts));
	$whoisserver = self::whoisservers($tld);

    if($whoisserver == '404' || !$whoisserver) {
		$message = "No appropriate WHOIS server found for $domain's TLD (extension)";
	} else {
        $result = self::QueryWhoisServer($whoisserver, $domain);
        if(!$result) {
            $message = "We were not able to find any data from $whoisserver WHOIS registry for $domain. Check the domain name and try again in 30 seconds";
        }
        else if($result == 404) {
            $message = "We were not able to find any data from $whoisserver WHOIS registry for $domain. ";
        }
        else {
            ...
            $code = '200';
            $message = 'WHOIS Data retrieved from '.$whoisserver.' for '.$domain.'';
        }
    }

    $data_filters = explode(PHP_EOL, $result);

    $filtered = array();

    $check[] = 'domain_name';
    $check[] = 'registry_domain_id';
    $check[] = 'registrar_whois_server';
    $check[] = 'registrar_url';
    $check[] = 'updated_date';
    $check[] = 'creation_date';
    $check[] = 'registry_expiry_date';
    $check[] = 'registrar';
    $check[] = 'registrar_iana_id';
    $check[] = 'registrar_abuse_contact_email';
    $check[] = 'registrar_abuse_contact_phone';
    $check[] = 'domain_status';
    $check[] = 'name_server';


    foreach($data_filters as $key => $value) {
        $further_filtering = explode(": ", $value);
        $temp_data = strtolower(str_replace(' ', '_', $further_filtering[0]));
        if(in_array($temp_data, $check)) {

            if($temp_data == 'name_server') {
                for($i = 1; $i < 2; $i++) {
                    if(!isset($filtered['name_server_' . $i])) {
                        $filtered['name_server_'.$i] = $further_filtering[1];
                    }
                }
            }

            $filtered[$temp_data] = $further_filtering[1];
        }
    }
    ...
    return array(
        ...
        'ip' => $DNS['ip'],
        'cloud_provider' => $DNS['cloudhost'],
        'data_center' => $DNS['isp'],
        'country' => $DNS['country'],
        'latitude' => $DNS['latitude'],
        'longitude' => $DNS['longitude'],
...
public static function QueryWhoisServer($whoisserver, $domain) {
    ...
    if(!$fp) {
        // throw new \Exception('Error! Unable to find data for $domain');
        return 404;
    }
    ...
    public static function getWebsiteCountry($website) {
        $ch = curl_init('http://www.iplocationfinder.com/' . $website);
        ...
        preg_match('~Hostname:.*<~', $data, $cloudhost);
        preg_match('~Latitude:.*<~', $data, $lat);
        preg_match('~Longitude:.*<~', $data, $long);
        ...
        if(empty($cloudhost)) {
            $cloudhost = 'Not Available';
        } else {
            //Cloudhost clean data
            $cloudhost = explode(':', strip_tags($cloudhost[0]));
            $cloudhost = $cloudhost[1];
        }
        ...
        if(empty($lat) || empty($long)) {
            ...
            $long = explode(':', strip_tags($long[0]));
            $long = $long[1];
        }
