...
class Whois
{
    private $WHOIS_SERVERS = array(
        ...
        "tech" => array("whois.nic.tech", "/Creation Date:(.*)/", "/Updated Date:(.*)/", "/Registry Expiry Date:(.*)/"),
    ...
    private function queryWhois($server, $domain) {
        ...
        if($serverIP != ''){
            ...
            if($extraCom) {
                ...
                if(!defined('TMP_DIR'))
                    define('TMP_DIR', 'temp' . DIRECTORY_SEPARATOR);


                if(File::isDirectory(TMP_DIR)) {
                    File::makeDirectory(TMP_DIR, 0777, true, true);
                }


                $queryFile = public_path('storage' . DIRECTORY_SEPARATOR . TMP_DIR . 'query.txt');

                // dd($serverIP);

                self::putMyData($queryFile, $domain . "\r\n");
    ...
    public static function host_info($site) {
